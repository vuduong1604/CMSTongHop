import { _decorator, Component, Node, Sprite, Texture2D, SpriteFrame, assetManager, EditBox, Toggle, AudioSource, AudioClip, ImageAsset } from 'cc';
import { Data } from './Data';
import { IDGame, InforServer } from './InforServer';
import { ItemData } from './ItemData';
const { ccclass, property } = _decorator;

@ccclass('ItemGame14ngang')
export class ItemGame14ngang extends Component {
  ...
  GetData() {
    ...
    itemdata.GameIdMini = IDGame.Game14ngang;
  ...
  async LoadData(data) {
    ...
    console.log("Game 1 4 ngang Data =>", JSON.parse(data));
    ...
    assetManager.loadRemote<ImageAsset>(url_imagequest, { ext: '.png' }, (err, imageAsset) => {
      ...
      this.Question.getChildByName("Mask").getChildByName("Sprite").getComponent(Sprite).spriteFrame = spriteFrame;
    ...
    dataObj.answer.map((el, index) => {
      ...
      assetManager.loadRemote<ImageAsset>(url, { ext: '.png' }, (err, imageAsset) => {
        ...
        texture.image = imageAsset;
        spriteFrame.texture = texture;
        listNodeAns[i].getChildByName("Mask").getChildByName("Sprite").getComponent(Sprite).spriteFrame = spriteFrame;
